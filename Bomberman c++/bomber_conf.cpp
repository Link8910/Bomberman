...
char Jugador::colocar_posiciones(char a[18][39])
	{
		...
		if (p_coord[0] == b_coord[0] && p_coord[1] == b_coord[1] && bomba_activado == true)
		{
...
char Jugador::borrar_posiciones(char a[18][39])
	{
		if (a[p_coord[0]][p_coord[1]] == '8' && bomba_activado == true)
		{
...
bool Jugador::pasos(int p_y, int p_x, int coordenadas[24][6][2])
	{
		for (int i = 0; i < 24; i++)
		{
			for (int j = 0; j < 6; j++)
			{
				if (coordenadas[i][j][0] == p_x && coordenadas[i][j][1] == p_y)

					return false;


				if (p_y == -1 || p_y == 18 || p_x == -1 or p_x == 39)
					return false;
...

	{
		...
		if (contador_flama == 1)
		{
			...
			for (int i; i < alcance; i++)
			{
				...
				if (down == true && pasos(b_coord[0] + i, b_coord[1], coordenadas) == true)
				{
				...
				if (up == true && pasos(b_coord[0] - i, b_coord[1], coordenadas) == true)
				{
					a[b_coord[0] - i][b_coord[1]] = 'x';
				}
				...
				if (left == true && pasos(b_coord[0], b_coord[1] - i, coordenadas) == true)
				{
				...
				if (right == true && pasos(b_coord[0], b_coord[1] + i, coordenadas) == true)
				{
					a[b_coord[0]][b_coord[1] + i] = 'x';
				}
...
void Mapa::reloj(bool &vida)
	{		
		...
		contador ++;
		if (minutos == 0 && segundos == 0) (vida ) = false;
	}
...
void Mapa::imprime(bool &vida, bool Escena_boss, int vida_boss)
	{
		this->reloj(vida);
		for (int i =0 ; i < 1; i++)
		{
...
Enemigo::Enemigo(int x, int y, int velocidad)
	{
		vida = true;
		temp_velocidad = 0;//
		e_coord[0] = x;
...
bool Enemigo::pasos(int p_y, int p_x, int coordenadas[24][6][2])
	{
		for (int i = 0; i < 24; i++)
		{
			for (int j = 0; j < 6; j++)
			{
				...
				if (p_y == -1 or p_y == 18 || p_x == -1 || p_x == 39)
					return false;
...
char Enemigo::colocar_enemigo(char a[18][39], int coordenadas[24][6][2], int x, int y)
	{
		if (vida == true)
		{
			...
			else if (a[e_coord[0]][e_coord[1]] == 'x') a[e_coord[0]][e_coord[1]] = 'x';
...
Enemigo_simple::Enemigo_simple(int x, int y,int cansancio, int velocidad,int energia ):Enemigo(x,y,velocidad)
	{
		
		this->cansancio = cansancio;
		this->energia = energia;
	}
char Enemigo_simple::colocar_enemigo(char a[18][39], int coordenadas[24][6][2], int x, int y)
	{
		if (vida == true)
		{
			...
			if (cansancio == 30) cansancio = 0;cansancio++;
...
Boss::Boss(int x, int y,int velocidad):Enemigo(x,y,velocidad)
	{
		boss_escena = false;
		cantidad_vida = 100;
	}
	
char Boss::muerto(char a[18][39], int x, int y)
	{
		if (a[x][y] == 'x')
		{
			cantidad_vida = cantidad_vida-1;
			if (cantidad_vida >= 75) velocidad = 3;
			if ( 15 <= cantidad_vida   && cantidad_vida < 75 ) velocidad = 2;
			if ( 15 > cantidad_vida) velocidad =1;
		}
		if (cantidad_vida == 0) vida = false;
	}
...
bool Menu::menuPri(){
		...
		cout << "      ____   " << endl;	
		cout << "     /    )                /                                  " << endl;
		cout << "    /__  /   ___   _ __   /__    __   __   _  _    __   __    " << endl;
		cout << "   /    )   /   ) / /  ) /   ) /___) /  ) / /  ) /   ) /  )" << endl;
